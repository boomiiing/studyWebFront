...
console.log(params)//{ name: 'John', age: '30', hobbies: 'swimming' }
 ...
 #### 1. 数据劫持
- 使用 Object.defineProperty() 劫持数据的 getter 和 setter
- 在 getter 中收集依赖，在 setter 中触发更新
  ...
  <script>
  function deserve(obj){
      Object.keys(obj).forEach(ele=>{
          let value = obj[ele]
          Object.defineProperty(obj,ele,{
              get(){
                  console.log('有人读了'+value);
                  return value
              },
              set(newValue){
                  if(value != newValue){
                      console.log('有人写了'+newValue);
                      value = newValue
                      update()
                  }
              }
  ...
  //实现数据劫持，循环为数据绑定get,set方法
  deserve(modelObj)
  update()
  function update(){
      let nameOne = document.getElementById('nameOne')
      //读取name触发get方法
      nameOne.innerText = modelObj.name
  }
  let inputName = document.getElementById('inputName')
  inputName.addEventListener('input',(e)=>{
      console.log(e);
      //为name赋值触发set
      modelObj.name = e.target.value
  })
...
### 14.vue3数据双向绑定实现
 #### 1. 数据劫持
 - 相比 Object.defineProperty，Proxy 提供了更强大和全面的数据劫持能力。
```js
<body>
    <h1 id="nameOne"></h1>
    <input type="text" id="inputName">
...
<script>
    function reactive(obj) {
        return new Proxy(obj, {
            get(target, property) {
                console.log('有人读了' + property);
                return target[property]
            },
            set(target, property, newValue) {
                console.log(`设置 ${property} = ${newValue}`);
    ...
    const proxyObj = reactive(modelObj);
    let nameOne = document.getElementById('nameOne')
    upadate()
    function upadate(){
        nameOne.innerText = proxyObj.name
    }
    let inputName = document.getElementById('inputName')
    inputName.addEventListener('input',(e)=>{
...
<script>
    let count = 0
    let lightArr = ['yellow', 'red', 'green']
    let timerArr = [3, 10, 10]
    function trfficLight() {
        setTimeout(() => {
            count++
            console.log(count);
            console.log(lightArr[count % 3] + "");
            let allLight = document.getElementsByClassName('item')
            for (let i = 0; i < allLight.length; i++) {
                allLight[i].style.background = "#fff";
            }

            let currentLight = document.getElementsByClassName(lightArr[count % 3] + "")
            console.log(currentLight);
            currentLight[0].style.background = lightArr[count % 3] + ""
            trfficLight()

        }, timerArr[count % 3] * 1000)
    }
    trfficLight()
</script>
```
...
function snakeToCamel(str){
  //_([a-z]) 是整个匹配模式,即match
  //([a-z]) 是一个捕获组，用于捕获下划线后的字母，即letter
...
#### 1.使用 Fisher-Yates 洗牌算法
 ...
 - 洗牌算法从数组的末尾往前循环，获取包含自身位置以前的随机一位，进行替换，从而实现打乱顺序返回数组
